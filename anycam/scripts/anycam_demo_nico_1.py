...
def main(cfg: DictConfig):
    ...
    if output_path and not export_colmap:
        ...
        if hasattr(proj, 'cpu'):
            ...
            np.save(output_path / "projection.npy", np.array(proj))
        ...
        # Salva frames e keyframes se disponibili
        if 'frames' in locals():
            np.save(output_path / "frames.npy", np.array(frames))
        if 'keyframes' in locals():
            np.save(output_path / "keyframes.npy", np.array(keyframes))
