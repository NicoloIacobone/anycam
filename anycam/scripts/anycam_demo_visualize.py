...
def plot_to_rerun(
    ...
    for id in range(len(trajectory)):
        ...
        pose = trajectory[id]
        # Support both torch.Tensor and np.ndarray
        if hasattr(pose, 'cpu'):
            pose_cpu = pose.cpu()
            rot = pose_cpu[:3, :3].numpy()
            trans = pose_cpu[:3, 3].numpy()
        else:
            rot = pose[:3, :3]
            trans = pose[:3, 3]
        ...
        rr.log(f"world/scene/active_cam", rr.Transform3D(translation=trans, mat3x3=rot, axis_length=0.01))

        # Camera trajectory line
        if hasattr(trajectory, 'cpu'):
            cam_traj = [p[:3, 3].cpu().numpy().tolist() for p in trajectory[:id+1]]
        else:
            cam_traj = [p[:3, 3].tolist() for p in trajectory[:id+1]]
        rr.log("world/scene/cam_traj", rr.LineStrips3D(cam_traj, colors=[(0, 255, 0)]), static=False)

        rr.log("world/scene/active_cam/input", rr.Image((imgs[id] * 255).astype(np.uint8)).compress(jpeg_quality=95))
        
        rr.log("world/scene", rr.Transform3D(translation=trans, mat3x3=rot, axis_length=0, from_parent=True))

        if keyframes is None or id in keyframes:
            # kid = keyframes.index(id) if keyframes is not None else id

            kid = np.where(keyframes == id)[0][0] if keyframes is not None else id

            # Convert everything to torch.Tensor on cuda for lift_image
            img = imgs[id]
            depth = depths[kid]
            # Resize img if shape does not match depth
            if img.shape[:2] != depth.shape[-2:]:
                import cv2
                img_resized = cv2.resize(img, (depth.shape[-1], depth.shape[-2]))
                if img_resized.ndim == 2:
                    img_resized = img_resized[..., None]
                img_t = torch.tensor(img_resized).cuda()
            else:
                img_t = torch.tensor(img).cuda() if not isinstance(img, torch.Tensor) else img.cuda()
            depth_t = torch.tensor(depth).cuda() if not isinstance(depth, torch.Tensor) else depth.cuda()
            pose_t = torch.tensor(trajectory[id]).cuda() if not hasattr(trajectory[id], 'cuda') else trajectory[id].cuda()
            proj_t = torch.tensor(proj).cuda() if not isinstance(proj, torch.Tensor) else proj.cuda()

            # Check shape compatibility before calling lift_image
            if img_t.shape[:2] != depth_t.shape[-2:]:
                print(f"[WARNING] Skipping frame {id}: image shape {img_t.shape[:2]} != depth shape {depth_t.shape[-2:]}")
                continue

            pts, colors = lift_image(img_t, depth_t, pose_t, proj_t)
            mask = filter_depth(depth_t, threshold=filter_depth_threshold)

            mask = mask.view(-1)

            if max_depth > 0:
                mask = mask & depth_t.view(-1)
            ...
            if uncertainties is not None:
                unc = uncertainties[kid]
                unc_t = torch.tensor(unc).cuda() if not isinstance(unc, torch.Tensor) else unc.cuda()
                uncertainty_img = color_tensor((unc_t / uncertainty_thresh).clamp(0, 1), cmap="plasma", norm=False)[0]
...
def main(cfg: DictConfig):
    ...
    """
    input_path = cfg.get("input_path", "/scratch2/nico/examples/nicola.mp4")
    output_path = cfg.get("output_path", None)
    model_path = cfg.get("model_path", "/scratch2/nico/anycam/pretrained_models/anycam_seq8")
    ...
    results_dir = "/scratch2/nico/anycam/examples/results/bundle_adjustment"

    trajectory = np.load(f"{results_dir}/trajectory.npy", allow_pickle=True)
    depths = np.load(f"{results_dir}/depths.npy", allow_pickle=True)
    uncertainties = np.load(f"{results_dir}/uncertainties.npy", allow_pickle=True)
    proj = np.load(f"{results_dir}/projection.npy", allow_pickle=True)
    keyframes = np.load(f"{results_dir}/keyframes.npy", allow_pickle=True)
    frames = np.load(f"{results_dir}/frames.npy", allow_pickle=True)

    # Visualization or export
    vis_config = cfg.get("vis", {})
    
    print(f"Visualizing results with rerun (mode: {rerun_mode})...")
    print(f"[DEBUG] Visualization config: {vis_config}")
    plot_to_rerun(
