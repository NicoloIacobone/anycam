...
def main(cfg: DictConfig):
    ...
    print(f"[DEBUG] visualize: {visualize}")
    print(f"[DEBUG] rerun_mode: {rerun_mode}")
    print(f"[DEBUG] export_colmap: {export_colmap}")
    print(f"[DEBUG] image_size: {image_size}")
    print(f"[DEBUG] ba_refinement: {ba_refinement}")
    print(f"[DEBUG] target_fps: {target_fps}")
    ...
    # Process frames
    print("[DEBUG] Starting process_video()")
    ...
    print("[DEBUG] process_video() finished")
    ...
    print(f"[DEBUG] Depths length: {len(depths)}")
    ...
    if not ba_refinement:
        ...
        print(f"[DEBUG] Uncertainties shape (no BA): {uncertainties.shape}")
    else:
        keyframes = [i * 3 for i in range(len(trajectory) // 3)]
        uncertainties = extras_dict["ba_uncertainties"]
        read_frames = frames
        print(f"[DEBUG] Uncertainties shape (BA): {uncertainties.shape if hasattr(uncertainties, 'shape') else type(uncertainties)}")

    uncertainties = torch.cat((uncertainties, uncertainties[-1:]), dim=0)
    print(f"[DEBUG] Uncertainties shape after cat: {uncertainties.shape}")
    ...
    if output_path and not export_colmap:
        ...
        print(f"[DEBUG] Saved trajectory.npy shape: {trajectory_np.shape}")

        # Save projection matrix
        np.save(output_path / "projection.npy", proj.cpu().numpy())
        print(f"[DEBUG] Saved projection.npy shape: {proj.cpu().numpy().shape}")
        ...
        if depths is not None:
            ...
            print(f"[DEBUG] Saved depths.npy shape: {depths_np.shape}")
        ...
        if uncertainties is not None:
            ...
            print(f"[DEBUG] Saved uncertainties.npy shape: {uncertainties_np.shape}")
