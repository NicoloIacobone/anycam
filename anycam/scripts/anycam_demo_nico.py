...
def main(cfg: DictConfig):
    ...
    trajectory = np.load("/scratch2/nico/anycam/examples/results/trajectory.npy", allow_pickle=True)
    
    # Extract depth and uncertainty information
    # best_candidate = extras_dict["best_candidate"]
    # depths = extras_dict["seq_depths"]
    depths = np.load("/scratch2/nico/anycam/examples/results/depths.npy", allow_pickle=True)

    if not ba_refinement:
        print("No BA refinement")
        read_frames = frames
    ...
    uncertainties = np.load("/scratch2/nico/anycam/examples/results/uncertainties.npy", allow_pickle=True)

    proj = np.load("/scratch2/nico/anycam/examples/results/projection.npy", allow_pickle=True)
